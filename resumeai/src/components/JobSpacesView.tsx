...
export default function JobSpacesView({ onBack }: { onBack: () => void }) {
  const [spaces, setSpaces] = useState<JobSpaceSummary[]>([]);
  const [loadingList, setLoadingList] = useState(true);
  const [listError, setListError] = useState<string | null>(null);

  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [detailSpace, setDetailSpace] = useState<JobSpaceSummary | null>(null);
  ...
  const openSpace = async (id: string) => {
    setSelectedId(id);
    setFormError(null);
  ...
  const maxSpaces = 5;
  const atLimit = spaces.length >= maxSpaces;
    ...
    return (
      <div className="max-w-3xl mx-auto">
        ...
        >
          ...
          <h3 className="text-lg font-semibold text-white mb-4">Job links</h3>
          {loadingDetail ? (
            ...
            <ul className="space-y-3">
              {links.map((l) => (
                ...
                >
                  ...
                  {l.title && (
                    <p className="text-xs text-slate-500 mt-1 break-all">{l.url}</p>
                  )}
                  {l.note && <p className="text-slate-400 text-sm mt-2">{l.note}</p>}
                  <p className="text-slate-600 text-xs mt-2">
                    by {l.user_id === myId ? 'You' : shortUser(l.user_id)} ·{' '}
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {listError && (
        <div className="mb-6 p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-400 text-sm">
          ...
          <p className="text-rose-400/80 text-xs mt-2">
            Start the API locally: <code className="text-rose-300">cd backend && uvicorn main:app --reload</code>
          </p>
        </div>
      )}

      <div className="mb-10">
        <h2 className="text-3xl font-black text-white tracking-tight mb-2">Job spaces</h2>
        <p className="text-slate-400 text-sm leading-relaxed max-w-xl">
          Create a space, share the invite code, and collect job posting links together. You can be in up to{' '}
          {maxSpaces} spaces at once (created or joined).
        </p>
        <p className="text-slate-600 text-xs mt-2">
          Spaces: <span className="text-slate-400">{spaces.length}</span> / {maxSpaces}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        ...
        >
          <h3 className="text-white font-bold mb-4">Create a space</h3>
          <input
            type="text"
          ...
          >
            {atLimit ? `Limit reached (${maxSpaces} spaces)` : 'Create space'}
          </button>
        </motion.form>
        ...
        <motion.form
          ...
          className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6"
        >
          <h3 className="text-white font-bold mb-4">Join with code</h3>
          <input
            type="text"
      ...
      <h3 className="text-lg font-semibold text-white mb-4">Your spaces</h3>
      {loadingList ? (
